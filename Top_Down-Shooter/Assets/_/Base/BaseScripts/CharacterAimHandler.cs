...
//                transform.Find("ShootLight").position = shootFlashPosition;
//                transform.Find("ShootLight").gameObject.SetActive(true);
//                FunctionTimer.Create(() => transform.Find("ShootLight").gameObject.SetActive(false), .04f, "ShootLight", false, true);

//                // Any enemy hit?
//                raycastHit = Physics2D.Raycast(shootFlashPosition, (targetPosition - shootFlashPosition).normalized, Vector3.Distance(shootFlashPosition, targetPosition));
//                if (raycastHit.collider != null) {
